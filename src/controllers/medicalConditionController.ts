...
import MedicalCondition from "../models/catalog/MedicalCondition";


const asyncHandler = require("express-async-handler");
const modelTitle = "Medical Condition";

export const getAllMedicalConditions = asyncHandler(
    ...
    try {
        ...
        const data = await MedicalCondition.find(query)
            .sort({ [sortBy as string]: sortOrder })
            .skip((parsedPage - 1) * parsedLimit)
            .limit(parsedLimit);

        // Get the total number of documents
        const totalData = await MedicalCondition.countDocuments(query);
