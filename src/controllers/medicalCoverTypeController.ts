...
import MedicalCoverType from "../models/catalog/MedicalCoverType";

const asyncHandler = require("express-async-handler");
const modelTitle = "Medical Cover Types";

export const getAllMedicalCoverTypes = asyncHandler(
    ...
    try {
        ...
        const data = await MedicalCoverType.find(query)
            .sort({ [sortBy as string]: sortOrder })
            .skip((parsedPage - 1) * parsedLimit)
            .limit(parsedLimit);

        // Get the total number of documents
        const totalData = await MedicalCoverType.countDocuments(query);
