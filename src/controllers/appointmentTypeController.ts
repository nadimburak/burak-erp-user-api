...
import AppointmentType from "../models/appointment/AppointmentType";

const asyncHandler = require("express-async-handler");
const modelTitle = "Appointment Types";

export const getAllAppointmentTypes = asyncHandler(
    ...
    try {
        ...
        const data = await AppointmentType.find(query)
            .sort({ [sortBy as string]: sortOrder })
            .skip((parsedPage - 1) * parsedLimit)
            .limit(parsedLimit);

        // Get the total number of documents
        const totalData = await AppointmentType.countDocuments(query);
