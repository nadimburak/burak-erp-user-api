...
import MaritalStatus from "../../models/catalog/MaritalStatus";

const asyncHandler = require("express-async-handler");

const modelTitle = "Marital Status";

export const getAllMaritalStatus = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      const data = await MaritalStatus.find(query)
        .sort({ [sortBy as string]: sortOrder })
        .skip((parsedPage - 1) * parsedLimit)
        .limit(parsedLimit);

      // Get the total number of documents
      const totalData = await MaritalStatus.countDocuments(query);
