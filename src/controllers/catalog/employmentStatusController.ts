...
import EmploymentStatus from "../../models/catalog/EmploymentStatus";

const asyncHandler = require("express-async-handler");

const modelTitle = "Employment Status";

export const getAllEmploymentStatus = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      const data = await EmploymentStatus.find(query)
        .sort({ [sortBy as string]: sortOrder })
        .skip((parsedPage - 1) * parsedLimit)
        .limit(parsedLimit);

      // Get the total number of documents
      const totalData = await EmploymentStatus.countDocuments(query);
