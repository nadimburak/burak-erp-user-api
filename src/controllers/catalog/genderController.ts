...
import Gender from "../../models/catalog/Gender";

const asyncHandler = require("express-async-handler");

const modelTitle = "Gender";

export const getAllGenders = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      const data = await Gender.find(query)
        .sort({ [sortBy as string]: sortOrder })
        .skip((parsedPage - 1) * parsedLimit)
        .limit(parsedLimit);

      // Get the total number of documents
      const totalData = await Gender.countDocuments(query);
