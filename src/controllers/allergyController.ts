...
import Allergy from "../models/catalog/Allergy";


const asyncHandler = require("express-async-handler");
const modelTitle = "Allergies";

export const getAllAllergies = asyncHandler(
    ...
    try {
        ...
        const data = await Allergy.find(query)
            .sort({ [sortBy as string]: sortOrder })
            .skip((parsedPage - 1) * parsedLimit)
            .limit(parsedLimit);

        // Get the total number of documents
        const totalData = await Allergy.countDocuments(query);
